...
10. **Flash-boot capability is bootloader-build-dependent** (proven by matrix test on lan3): some units' U-Boot builds cannot `sf read`+`bootm` ANY image while `tftpboot`+`bootm` works. Record each unit's bootloader build (env `CURR_VER`) in inventory; incapable units are permanently `run boot_net`-dependent — their VLAN's TFTP server is life support.
