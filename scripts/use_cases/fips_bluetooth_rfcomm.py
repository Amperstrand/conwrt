...
from profile.ops import Op, ServiceAction, ShellCommand, render_shell
...
def _resolve_params(params: dict[str, Any]) -> dict[str, Any]:
    role = str(params.get("role", "client"))
    channel = int(params.get("channel", 1))
    tun_enabled = bool(params.get("tun_enabled", True))
    tun_dns = bool(params.get("tun_dns", True))
    tun_gateway = bool(params.get("tun_gateway", True))
    bt_pin = str(params.get("bt_pin", "0000"))
    peers = list(params.get("peers", []))
    return {
        "role": role,
        "channel": channel,
...
def _build_yaml_parts(r: dict[str, Any]) -> list[str]:
    """Build FIPS YAML config lines (shared between shell and ops builders)."""
    role = r["role"]
    channel = r["channel"]
    tun_enabled = r["tun_enabled"]
    tun_dns = r["tun_dns"]
    tun_gateway = r["tun_gateway"]
    peers = r["peers"]
...
def _build_fips_rfcomm_ops(params: dict[str, Any]) -> list[Op]:
    ...
    ops: list[Op] = [ShellCommand(command="mkdir -p /etc/fips")]

    for cmd in _echo_write("/etc/fips/fips.yaml", yaml_content):
        ops.append(ShellCommand(command=cmd))

    ops.append(ShellCommand(command=f"echo '{bt_pin}' > /etc/fips/bt-pin"))

    if role == "server" and peers_allow_content.strip():
        for cmd in _echo_write("/etc/fips/peers.allow", peers_allow_content):
            ops.append(ShellCommand(command=cmd))

    for cmd in _echo_write("/etc/init.d/bt-agent", bt_agent_init):
        ops.append(ShellCommand(command=cmd))
    ops.append(ShellCommand(command="chmod +x /etc/init.d/bt-agent"))
    ops.append(ServiceAction(name="bt-agent", action="enable"))

    for cmd in _echo_write("/etc/init.d/fips", fips_init):
        ops.append(ShellCommand(command=cmd))
    ops.append(ShellCommand(command="chmod +x /etc/init.d/fips"))
    ops.append(ServiceAction(name="fips", action="enable"))
    ops.append(ShellCommand(command=f"echo 'FIPS Bluetooth RFCOMM configured ({role}, channel {channel}, {len(peers)} peer(s))'"))
...
register(UseCase(
    ...
    build_configure=lambda p: render_shell(_build_fips_rfcomm_ops(p)),
    build_configure_ops=_build_fips_rfcomm_ops,
    packages_via="opkg",
    configure_via="ssh",
