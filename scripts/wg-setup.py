...
def build_uci_commands(pc: PeerConfig) -> list[str]:
    ...
    return [
        ...
        f"uci set network.wg0_peer.public_key='{pc.server_public_key}'",
        f"uci set network.wg0_peer.preshared_key='{pc.preshared_key}'",
        f"uci set network.wg0_peer.endpoint_host='{pc.endpoint_host}'",
        f"uci set network.wg0_peer.endpoint_port='{pc.endpoint_port}'",
