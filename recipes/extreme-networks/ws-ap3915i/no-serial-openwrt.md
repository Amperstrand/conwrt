...
> **Session status: COMPLETE.** Switch-initiated flash of AP3915i via GS1900-8HP PoE switch.
> OpenWrt 24.10.2 permanently installed, boots from SPI-NOR flash, overlayfs persisted through reboot.
> See `SWITCH-FLASH-PLAN.md` for the milestone plan (all M0-M6 complete).
...
## Switch-Initiated Flash — Completed (2026-05-23)

All milestones M0-M6 from `SWITCH-FLASH-PLAN.md` completed. The GS1900-8HP OpenWrt switch
orchestrated the entire flash process. Key post-flash steps:

### What was done after sysupgrade

1. **TFTP-booted initramfs** via modified CFG blocks (`bootcmd=run boot_net`)
2. **Ran sysupgrade -n** — OpenWrt 24.10.2 permanently written to SPI-NOR flash
3. **TFTP-booted initramfs again** (bootcmd still said TFTP)
4. **Loaded `kmod-mtd-rw`** — `insmod mtd-rw.ko i_want_a_brick=1` to bypass DTS read-only
5. **Wrote corrected CFG blocks** with `boot_openwrt` command via cross-compiled `mtd_raw_write`
...
| Component | Status |
|-----------|--------|
| **U-Boot bootloader** | Working — runs `bootcmd=run boot_openwrt; run boot_net` |
...
1. **OpenWrt Dropbear `authorized_keys` location**: `/etc/dropbear/authorized_keys`, NOT
   `~/.ssh/authorized_keys`. The Dropbear init script references this path explicitly.
2. **openssh-client can't read Dropbear-format private keys**: The switch's ed25519 key was
   generated by `dropbearkey` and the openssh SSH client produces "error in libcrypto: unsupported".
   Use `dbclient` (Dropbear native) or `sshpass` for password auth instead.
3. **`DROPBEAR_PASSWORD` env var**: `dbclient` accepts password via this environment variable,
   enabling non-interactive password auth from scripts.
4. **`sshpass` was available on the switch** despite the switch not having `opkg` — likely
   installed as a dependency or during the earlier tool installation phase.
5. **SSH algorithm compatibility**: Ubuntu's openssh client needs explicit algorithm flags to
   talk to the AP's Dropbear v2024.86:
   ```
   ...
   ```
6. **`ubi.mtd=0` in bootargs caused failure**: Stock CFG had this in bootargs/static_bootargs,
   causing the OpenWrt kernel to try UBI attach on CFG1 partition. Removing it fixed flash boot.

## What We Still Need To Do

1. **Test WiFi radios** work correctly
2. **Verify the ART partition** (radio calibration data) survived
