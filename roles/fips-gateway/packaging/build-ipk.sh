...
#
# Usage:
#   ./build-ipk.sh [--arch <name>] [--source <fips-repo>] [--branch <git-branch>]
#
# Arguments:
#   --source <dir>   Path to the FIPS source repository (default: auto-detect)
#   --branch <name>  Git branch to checkout before building (default: current)
#
# Architectures (--arch):
...
FIPS_SOURCE=""    # if set, overrides auto-detected FIPS source path
GIT_BRANCH=""     # if set, checkout this branch before building
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --source) FIPS_SOURCE="$2"; shift 2 ;;
        --source=*) FIPS_SOURCE="${1#*=}"; shift ;;
...
if [ -n "$FIPS_SOURCE" ]; then
    PROJECT_ROOT="$(cd "$FIPS_SOURCE" && pwd)"
else
    PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
fi

if [ -n "$GIT_BRANCH" ] && [ -d "$PROJECT_ROOT/.git" ]; then
    echo "==> Checking out branch '$GIT_BRANCH' in $PROJECT_ROOT..."
    git -C "$PROJECT_ROOT" checkout "$GIT_BRANCH" 2>/dev/null || true
    git -C "$PROJECT_ROOT" pull --ff-only 2>/dev/null || true
fi
...
Description: FIPS Mesh Network Daemon (Gateway Role)
 Distributed mesh networking over UDP, TCP, and raw Ethernet.
 Gateway variant: fips-gateway service enabled by default with
 proxy_ndp, IPv6 forwarding, and virtual IP pool for LAN clients.
